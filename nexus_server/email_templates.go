...
func primaryBtn(label, href string) string {
	return `<a href="` + href + `" style="display:inline-block;background:#a677ff;color:#fff;text-decoration:none;font-weight:700;font-size:15px;padding:14px 32px;border-radius:12px;letter-spacing:-0.01em">` + label + `</a>`
}
...
func codeBlock(code string) string {
    ...
    <span style="font-family:'JetBrains Mono','Courier New',monospace;font-size:28px;font-weight:700;letter-spacing:0.15em;color:#a677ff">` + code + `</span>
  </div>`
}
...
func emailVerification(username, verifyLink, code string) string {
    ...
    <p style="margin:0 0 28px;font-size:15px;color:#98989d;line-height:1.6">Hey ` + username + `, welcome to Phaze. Tap the button to activate your account.</p>
...
func emailPasswordReset(username, resetLink string) string {
    ...
    <p style="margin:0 0 28px;font-size:15px;color:#98989d;line-height:1.6">Hey ` + username + `, we got a request to reset your Phaze password. Tap the button — this link is valid for <strong style="color:#f5f5f7">1 hour</strong>.</p>
...
func emailInvite(inviterUsername, joinLink string) string {
	display := strings.TrimSpace(inviterUsername)
    ...
    <div style="text-align:center;margin:0 0 28px">` + primaryBtn("Join Phaze", joinLink) + `</div>` +
		mutedText(`Or paste this link in your browser: <a href="` + joinLink + `" style="color:#a677ff;text-decoration:none">` + joinLink + `</a>`)
	return emailBase(display+" invited you to Phaze.", content)
}
...
func emailSupporterThankYou(name string) string {
    ...
    <p style="margin:0 0 20px;font-size:15px;color:#98989d;line-height:1.6">Hey ` + name + `, your support means everything to us. Once your contribution is confirmed, your supporter badge will show up on your Phaze profile automatically.</p>
